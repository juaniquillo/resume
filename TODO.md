...
12. ◻️ I already worked on a select dropdown and added a `type` column to the resume_exports table for choosing another type of export for the CRUD. Work 1on a ProcessPdfExport job using the package `spatie/laravel-pdf`. Then rename the existing one to ProcessJsonExport.
13. ◻️ Add cache headers to the image's route.
...
16. ◻️ Work on a preview button that opens the biggest flux modal available and shows how the resume looks like. The button should be noticeable and should be placed on the top right corner on every page of the dashboard.